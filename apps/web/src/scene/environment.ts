...
function createHillRange(width: number, depth: number, amplitude: number, seed: string) {
...
function createHillsideSign(text: string): THREE.Mesh {
  ...
  const sign = new THREE.Mesh(
    new THREE.PlaneGeometry(17, 2.65),
...
function createSuspensionBridge(length: number): THREE.Group {
  ...
  for (const cz of [-0.78, 0.78]) {
    ...
    const cable = new THREE.Mesh(
      new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points), 60, 0.06, 6, false),
      orange.clone(),
    );
    group.add(cable);
    // Vertical hangers
    for (let i = 2; i < 39; i += 3) {
      const p = points[i];
      if (!p || p.y - deckHeight < 0.4) continue;
...
function createWindTurbine(): { group: THREE.Group; rotor: THREE.Group } {
  const group = new THREE.Group();
...
function createBayIsland(): THREE.Group {
  const group = new THREE.Group();
  const rock = new THREE.Mesh(
    new THREE.DodecahedronGeometry(2.2, 1),
    new THREE.MeshStandardMaterial({ color: 0x8f9a8a, roughness: 0.95, flatShading: true }),
  );
...
export function createEnvironment(group: THREE.Group): EnvironmentHandles {
  const waterMaterial = createWaterMaterial();
  const clouds: DriftSprite[] = [];
  const fogBanks: DriftSprite[] = [];
  const turbineRotors: THREE.Object3D[] = [];

  // Sun with layered glow
  const sunDisc = new THREE.Mesh(
    new THREE.SphereGeometry(5.4, 48, 24),
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: groundTexture, color: 0xf4ffef, roughness: 0.9, metalness: 0.02 }),
  );
  ...
  const base = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x5c4a38, roughness: 0.95 }),
  );
  base.position.set(-5, -0.76, 0);
  base.receiveShadow = true;
  group.add(base);
  const strata = new THREE.Mesh(
    new THREE.BoxGeometry(70.05, 0.3, 57.05),
    new THREE.MeshStandardMaterial({ color: 0x8a6f52, roughness: 0.9 }),
  );
  ...
  const beach = new THREE.Mesh(
    new THREE.PlaneGeometry(2.6, 57, 1, 8),
    new THREE.MeshStandardMaterial({ color: 0xf0dfb0, roughness: 0.95 }),
  );
  beach.rotation.x = -Math.PI / 2;
  beach.position.set(31.4, -0.05, 0);
  group.add(beach);
  const seawall = new THREE.Mesh(
    new THREE.BoxGeometry(1.0, 0.48, 58),
    new THREE.MeshStandardMaterial({ color: 0xb5c2c8, roughness: 0.72, metalness: 0.08 }),
  );
  ...
  const farMaterial = farHills.material as THREE.MeshStandardMaterial;
