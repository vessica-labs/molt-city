...
import type { Building, BuildingType, Company, Lot } from '@molt-city/shared';
import {
  companyBrand,
  createBillboardTexture,
  createFacadeSignTexture,
  createTextSprite,
  hashFloat,
  hashPick,
  seededChance,
...
export function buildingHeight(building: Building) {
...
export function renderedRoofline(building: Building, computedHeight: number) {
...
export function companyHeight(building: Building, company: Company) {
...
export function companyDevelopmentScore(company: Company) {
...
export function companiesByLotDevelopment(companies: Company[]) {
...
export function companyColor(archetype: Company['archetype']) {
...
function addWindowGrid(group: THREE.Group, width: number, height: number, depth: number, baseY: number, seed: string, offsetX = 0, offsetZ = 0) {
...
function addFloorBands(group: THREE.Group, width: number, height: number, depth: number, baseY: number, color: number) {
...
function addEntrance(group: THREE.Group, width: number, depth: number, accent: number) {
...
function addParapet(group: THREE.Group, width: number, depth: number, height: number, color: number) {
...
function addRooftopClutter(group: THREE.Group, width: number, depth: number, height: number, seed: string) {
...
export function createRooftopBillboard(company: Pick<Company, 'name' | 'archetype' | 'stage'>, brand: BrandKit, faceWidth: number): THREE.Group {
  ...
  const frame = new THREE.Mesh(
    new THREE.BoxGeometry(faceWidth + 0.14, faceHeight + 0.14, 0.12),
    new THREE.MeshStandardMaterial({ color: failed ? 0x4a4d55 : 0x1b2342, roughness: 0.55, metalness: 0.4 }),
  );
  ...
  const backPanel = new THREE.Mesh(
    new THREE.PlaneGeometry(faceWidth, faceHeight),
    new THREE.MeshStandardMaterial({ color: 0x30364a, roughness: 0.7, metalness: 0.3 }),
  );
  ...
  if (!failed) {
    for (const x of [-faceWidth * 0.3, faceWidth * 0.3]) {
      ...
      const lamp = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0xfff2c4, emissive: 0xffe9a8, emissiveIntensity: 2 }),
      );
...
function addStageProps(group: THREE.Group, company: Company, width: number, depth: number, roofline: number, height: number, brand: BrandKit) {
  ...
  switch (company.stage) {
    ...
    case 'public-ish': {
      ...
      const spinner = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: brand.glow, emissive: brand.glow, emissiveIntensity: 0.9, roughness: 0.2, metalness: 0.5 }),
      );
      ...
      const pedestal = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0xd9e2ec, roughness: 0.3, metalness: 0.7 }),
      );
      pedestal.position.set(0, height + 0.9, 0);
      group.add(pedestal);
      // Gold parapet trim: subtle, tasteful, extremely not subtle
      const crownTrim = new THREE.Mesh(
        new THREE.BoxGeometry(width * 0.96, 0.14, depth * 0.96),
        new THREE.MeshStandardMaterial({ color: 0xffc94d, emissive: 0xa87908, emissiveIntensity: 0.35, roughness: 0.25, metalness: 0.8 }),
      );
...
function addCompanyGrowth(group: THREE.Group, building: Building, company: Company, width: number, depth: number, roofline: number, height: number, brand: BrandKit) {
  ...
  if (growth > 5.8 || company.stage === 'growth' || company.stage === 'public-ish') {
    const crown = new THREE.Mesh(
      new THREE.CylinderGeometry(width * 0.18, width * 0.26, 0.52, 6),
      new THREE.MeshStandardMaterial({ color: 0xeaffff, emissive: brand.glow, emissiveIntensity: 0.75, roughness: 0.28, metalness: 0.18 }),
    );
...
function addCompanyBranding(group: THREE.Group, company: Company, width: number, depth: number, height: number, brand: BrandKit) {
  ...
  billboard.position.set(
    (hashFloat(`${company.name}:bx`) - 0.5) * width * 0.4,
    height + boardHeight * 0.85 + 0.35 + lift,
    (hashFloat(`${company.name}:bz`) - 0.5) * depth * 0.4,
  );
  ...
  const facade = new THREE.Mesh(
    new THREE.PlaneGeometry(width * 0.86, width * 0.86 * (192 / 1024)),
    new THREE.MeshBasicMaterial({ map: createFacadeSignTexture(company.name, brand, company.stage === 'failed'), transparent: true, toneMapped: false }),
  );
...
export function createBuildingModel(building: Building, lot: Lot, company: Company | undefined, tick: number, lotWidth: number, lotDepth: number) {
  const group = new THREE.Group();
...
function createCoffeeShop(group: THREE.Group, building: Building, width: number, depth: number) {
  ...
  for (let i = 0; i < 6; i += 1) {
    const stripe = new THREE.Mesh(
      new THREE.CylinderGeometry(0.1, 0.1, width * 0.11, 8, 1, false, 0, Math.PI),
      new THREE.MeshStandardMaterial({ color: i % 2 ? 0xffffff : 0xff4d80, roughness: 0.6, side: THREE.DoubleSide }),
    );
...
function createResearchLab(group: THREE.Group, building: Building, width: number, depth: number, height: number) {
  ...
  const reactor = new THREE.Mesh(
    new THREE.CylinderGeometry(0.68, 0.68, height * 0.82, 28),
    new THREE.MeshStandardMaterial({ color: 0x9fffee, emissive: 0x40ffe0, emissiveIntensity: 0.9, transparent: true, opacity: 0.55, roughness: 0.18 }),
  );
...
function createCivicHall(group: THREE.Group, building: Building, width: number, depth: number) {
  ...
  const dome = new THREE.Mesh(
    new THREE.SphereGeometry(1.15, 28, 14, 0, Math.PI * 2, 0, Math.PI / 2),
    new THREE.MeshStandardMaterial({ color: 0xffd982, roughness: 0.3, metalness: 0.65, emissive: 0x7a5a10, emissiveIntensity: 0.2 }),
  );
...
function createTransitKiosk(group: THREE.Group, _building: Building, width: number, depth: number) {
  ...
  const tube = new THREE.Mesh(
    new THREE.CylinderGeometry(0.3, 0.3, width * 1.15, 16, 1, true),
    new THREE.MeshStandardMaterial({ color: 0xbfeaff, transparent: true, opacity: 0.4, roughness: 0.15, side: THREE.DoubleSide }),
  );
...
function createPonyMeadow(group: THREE.Group, _building: Building, width: number, depth: number) {
  ...
  for (let index = 0; index < 9; index += 1) {
    const flower = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: index % 3 === 0 ? 0xff66c4 : index % 3 === 1 ? 0xfff06b : 0xc99bff, emissive: 0xff66c4, emissiveIntensity: 0.15 }),
    );
  ...
  for (let index = 0; index < rainbowColors.length; index += 1) {
    const arc = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: rainbowColors[index] ?? 0xffffff, emissive: rainbowColors[index] ?? 0xffffff, emissiveIntensity: 0.45, transparent: true, opacity: 0.9 }),
    );
...
function createConcertShell(group: THREE.Group, building: Building, width: number, depth: number) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    const beam = new THREE.Mesh(
      new THREE.ConeGeometry(0.3, 2.0, 12, 1, true),
      new THREE.MeshBasicMaterial({ color: parColors[i] ?? 0xffffff, transparent: true, opacity: 0.12, depthWrite: false, side: THREE.DoubleSide }),
    );
...
function createMixedUseTower(group: THREE.Group, building: Building, width: number, depth: number, height: number) {
  ...
  for (const [w, y] of [[0.66, lowerH], [0.5, lowerH + midH]] as const) {
