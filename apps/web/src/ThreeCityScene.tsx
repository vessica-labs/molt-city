...
import { createEnvironment, type EnvironmentHandles } from './scene/environment';
import {
  createBlimp,
  createBoat,
  createCar,
...
class CityRenderer {
  ...
  private readonly bots: TrafficActor[] = [];
  private readonly boats: BoatActor[] = [];
  private readonly skyTraffic: SkyActor[] = [];
  private readonly people = new Map<string, NpcActor>();
  private readonly env: EnvironmentHandles;
  ...
  private tumbleweeds: THREE.Object3D[] = [];
  private fanBlades: THREE.Object3D[] = [];
  private orbitRings: THREE.Object3D[] = [];
  private transitPods: THREE.Object3D[] = [];
  ...
  private readonly animate = () => {
    ...
    for (const car of this.cars) updateTrafficActor(car, elapsed, 0.16);
    for (const bot of this.bots) updateTrafficActor(bot, elapsed, 0.16);
    ...
    for (const flyer of this.skyTraffic) {
      ...
      flyer.group.position.set(
        flyer.centerX + Math.cos(t) * flyer.radius,
        flyer.y + Math.sin(elapsed * 0.5 + flyer.phase) * 0.8,
        flyer.centerZ + Math.sin(t) * flyer.radius * 0.55,
      );
    ...
    for (const weed of this.tumbleweeds) {
      weed.position.x = (weed.userData.baseX as number) + Math.sin(elapsed * 0.7 + (weed.userData.seed as number)) * 1.1;
      weed.rotation.z = -elapsed * 2.4;
    }
    for (const blade of this.fanBlades) blade.rotation.y = elapsed * 6;
    for (const ring of this.orbitRings) {
      ring.rotation.z = elapsed * (0.6 + (ring.userData.seed as number) * 0.5);
    }
    for (const pod of this.transitPods) {
      pod.position.x = (pod.userData.baseX as number) + Math.sin(elapsed * 0.9 + (pod.userData.seed as number)) * 1.3;
    }
    for (const beacon of this.beacons) {
      const material = beacon.material as THREE.MeshStandardMaterial;
      material.emissiveIntensity = 1.4 + Math.sin(elapsed * 3 + beacon.id) * 0.9;
    }
    for (const flag of this.flags) flag.rotation.y = Math.sin(elapsed * 2.4) * 0.4;
    for (const rotor of this.env.turbineRotors) rotor.rotation.x = elapsed * 1.6;
  ...
  private setupTraffic() {
    ...
    const boatSpecs: Array<{ kind: 'sail' | 'ferry' | 'speed'; accent: number; baseX: number; baseZ: number; radiusX: number; radiusZ: number; speed: number }> = [
  ...
  private harvestAnimatedProps() {
    ...
    this.cityGroup.traverse((node) => {
      if (node.name === 'logo-spinner') this.spinners.push(node);
      ...
      } else if (node.name.startsWith('fan-blade')) this.fanBlades.push(node);
      ...
      } else if (node.name === 'beacon' && node instanceof THREE.Mesh) this.beacons.push(node);
      else if (node.name === 'civic-flag') this.flags.push(node);
    });
  }
...
function updateTrafficActor(actor: TrafficActor, elapsed: number, y: number) {
...
function disposeMaterial(material: THREE.Material) {
  for (const value of Object.values(material)) {
    if (value instanceof THREE.Texture && !value.userData.persistent) value.dispose();
  }
  material.dispose();
}
...
function createPreviewWorld(): WorldState {
  ...
  const companyPresets: Array<{ lotIndex: number; name: string; archetype: Company['archetype']; stage: Company['stage']; valuation: number; marketShare: number; research: number; cash: number }> = [
  ...
  const companies: Company[] = companyPresets.flatMap((preset, index) => {
    const lot = lots[preset.lotIndex];
