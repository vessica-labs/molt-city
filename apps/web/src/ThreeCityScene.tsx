...
import type { Building, BuildingType, CityEvent, Coordinates, Lot, Npc, WorldState } from '@molt-city/shared';
...
class CityRenderer {
  ...
  private setupEnvironment() {
    ...
    const ground = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ map: groundTexture, color: 0xffffff, roughness: 0.94, metalness: 0.02 }),
    );
    ...
    const base = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x37715e, roughness: 0.98 }),
    );
    ...
    const seawall = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xceddd8, roughness: 0.72, metalness: 0.08 }),
    );
  ...
  private addLot(lot: Lot, building: Building | undefined, tick: number) {
...
function createBuildingModel(building: Building, lot: Lot, tick: number) {
  const group = new THREE.Group();
