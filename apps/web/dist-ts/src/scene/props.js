...
export function createTree(kind, scale = 1, seed = 'tree') {
    ...
    if (kind === 'redwood') {
        ...
        for (let tier = 0; tier < 4; tier += 1) {
            ...
            const cone = new THREE.Mesh(new THREE.ConeGeometry(radius, 0.85 * scale, 8), new THREE.MeshStandardMaterial({ color: tones[tier % tones.length] ?? 0x25804f, roughness: 0.85, flatShading: true }));
        ...
        group.add(trunk);
        const body = new THREE.Mesh(new THREE.SphereGeometry(0.34 * scale, 8, 10), new THREE.MeshStandardMaterial({ color: 0x2c7a4b, roughness: 0.85, flatShading: true }));
        ...
        for (let i = 0; i < 3; i += 1) {
            const blob = new THREE.Mesh(new THREE.IcosahedronGeometry((0.4 - i * 0.06) * scale, 1), new THREE.MeshStandardMaterial({ color: tones[i] ?? 0x47b45e, roughness: 0.86, flatShading: true }));
...
function addWheels(group, length, width) {
...
export function createCar(color, robotaxi = false) {
    const group = new THREE.Group();
...
export function createDeliveryBot(seed) {
    const group = new THREE.Group();
    const shellColor = hashFloat(seed) > 0.5 ? 0xf7f9fc : 0xffd9e8;
    const shell = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.26, 0.24), new THREE.MeshStandardMaterial({ color: shellColor, roughness: 0.4, metalness: 0.1 }));
...
export function createBoat(kind, accent) {
    const group = new THREE.Group();
    if (kind === 'sail') {
...
export function createBlimp(texture) {
    const group = new THREE.Group();
    const envelope = new THREE.Mesh(new THREE.SphereGeometry(1.6, 24, 16), new THREE.MeshStandardMaterial({ color: 0xf2f6ff, roughness: 0.5, metalness: 0.05 }));
    envelope.scale.set(2.1, 0.85, 0.85);
    envelope.castShadow = true;
    group.add(envelope);
    for (const side of [1, -1]) {
        const banner = new THREE.Mesh(new THREE.PlaneGeometry(4.2, 1.05), new THREE.MeshBasicMaterial({ map: texture, transparent: true, side: THREE.DoubleSide }));
...
export function createHotAirBalloon() {
    ...
    for (let i = 0; i < 4; i += 1) {
        const slice = new THREE.Mesh(new THREE.SphereGeometry(1.15, 20, 14, (i / 4) * Math.PI * 2, Math.PI / 2), new THREE.MeshStandardMaterial({ color: stripes[i] ?? 0xff4d80, roughness: 0.6 }));
        slice.scale.y = 1.22;
        group.add(slice);
    }
...
export function createHvacUnit(scale = 1) {
    const group = new THREE.Group();
...
export function createWaterTank(scale = 1) {
    const group = new THREE.Group();
...
export function createSolarArray(cols, rows) {
    const group = new THREE.Group();
...
export function createAntennaMast(height, color) {
    const group = new THREE.Group();
...
// ---------------------------------------------------------------------------
export function createWorldSign(lines, style, width = 1.4, height = 0.7, postHeight = 0.55) {
    const group = new THREE.Group();
...
export function createPizzaBoxStack(count, seed) {
    const group = new THREE.Group();
...
export function createBeanBag(color) {
    const bag = new THREE.Mesh(new THREE.SphereGeometry(0.3, 10, 8), new THREE.MeshStandardMaterial({ color, roughness: 0.9, flatShading: true }));
...
export function createKombuchaKeg() {
    const group = new THREE.Group();
...
export function createFoodTruck(color, seed) {
    const group = new THREE.Group();
    ...
    group.add(awning);
    const menu = new THREE.Mesh(new THREE.PlaneGeometry(1.0, 0.32), new THREE.MeshBasicMaterial({ map: createGagSignTexture([hashFloat(seed) > 0.5 ? 'RAMEN BURRITO $23' : 'AI-POKE BOWL $27'], 'banner'), transparent: true }));
    menu.position.set(-0.1, 0.62, 0.34);
    group.add(menu);
    addWheels(group, 1.6, 0.7);
    return group;
}
export function createIpoBell() {
    const group = new THREE.Group();
...
export function createTumbleweed(scale = 1) {
    const weed = new THREE.Mesh(new THREE.IcosahedronGeometry(0.24 * scale, 1), new THREE.MeshStandardMaterial({ color: 0xb59a62, roughness: 1, wireframe: true }));
...
export function createCrane(height, color = 0xffb020) {
    const group = new THREE.Group();
...
export function createPony() {
    ...
    body.castShadow = true;
    group.add(body);
    for (const [lx, lz] of [[-0.32, -0.16], [-0.32, 0.16], [0.3, -0.16], [0.3, 0.16]]) {
...
export function createSteamPuff(scale) {
