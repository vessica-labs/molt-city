...
export function hashPick(items, seed) {
    const index = Math.floor(hashFloat(seed) * items.length) % items.length;
    return items[index];
}
export function seededChance(seed, index) {
...
// ---------------------------------------------------------------------------
export function drawCompanyLogo(ctx, cx, cy, size, archetype, seed, primary, secondary) {
    ctx.save();
    ctx.translate(cx, cy);
    const s = size / 2;
    switch (archetype) {
...
// ---------------------------------------------------------------------------
export function createFacadeSignTexture(name, brand, failed) {
    return cached(`facade|${name}|${failed}`, () => buildFacadeSignTexture(name, brand, failed));
}
function buildFacadeSignTexture(name, brand, failed) {
    const width = 1024;
    const height = 192;
    const { canvas, ctx } = makeCanvas(width, height);
    if (!ctx)
...
export function createGagSignTexture(lines, style) {
    return cached(`gag|${style}|${lines.join('¶')}`, () => buildGagSignTexture(lines, style));
}
function buildGagSignTexture(lines, style) {
    const width = 512;
    const height = 256;
    const { canvas, ctx } = makeCanvas(width, height);
    if (!ctx)
...
export function createCloudSprite(scale) {
...
export function createFogSprite(scale) {
